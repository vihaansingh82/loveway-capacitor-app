...
-- ------------------------------------------------------------
-- 29. Announcements — Birthday / Anniversary / Proposal / I Love You /
--     Special Moment / Custom wishes. Sender kisi ek profile ko target
--     karta hai; wo profile approve/reject kiye bina koi bhi kisi ke
--     naam se seedha publish nahi kar sakta (friendships ke pending/
--     accepted/declined pattern jaisa hi). Approve hote hi ek normal
--     post ban jaati hai (posts.kind='announcement', dono columns
--     posts table mein pehle se hain) — isse poora feed/like/comment
--     UI dobara bina kuch banaye reuse ho jaata hai.
-- ------------------------------------------------------------
create table if not exists public.announcements (
  ...
  kind          text not null default 'custom'
                check (kind in ('birthday', 'anniversary', 'proposal', 'love', 'special_moment', 'custom')),
  message       text,
  media_url     text,
  visibility    text not null default 'friends'
                check (visibility in ('public', 'friends', 'partner', 'private')),
  status        text not null default 'pending'
                check (status in ('pending', 'approved', 'rejected')),
  post_id       bigint references public.posts (id) on delete set null,
  created_at    timestamptz not null default now(),
  responded_at  timestamptz,
  constraint announcement_not_self check (sender_id <> recipient_id)
);
create index if not exists announcements_recipient_idx on public.announcements (recipient_id, status);
create index if not exists announcements_sender_idx    on public.announcements (sender_id, status);
...
create policy announcements_select on public.announcements
  for select to authenticated
  using (sender_id = auth.uid() or recipient_id = auth.uid() or public.lw_is_admin());

create policy announcements_insert on public.announcements
  for insert to authenticated
  with check (sender_id = auth.uid() and status = 'pending');

-- sirf jise announcement mila hai wahi approve/reject kar sakta hai
...
create or replace function public.lw_notify_announcement()
returns trigger language plpgsql security definer
set search_path = public as $$
declare
  v_post_id  bigint;
...
begin
  if tg_op = 'INSERT' then
    perform public.lw_notify(new.recipient_id, new.sender_id,
      'announcement_request', 'Naya announcement — approve karo', null, 'announcements.html');
    return new;
  end if;
  ...
  if tg_op = 'UPDATE' and new.status = 'approved' and old.status = 'pending' then
    v_title := case new.kind
      ...
      when 'anniversary'    then '💕 Happy Anniversary!'
      when 'proposal'       then '💍 Will you...?'
      when 'love'           then '❤️ I Love You'
      when 'special_moment' then '🎉 A Special Moment'
      else '💌 A message for you'
    end;

    insert into public.posts (author_id, dedicated_to, kind, content, media_url, visibility)
    values (new.sender_id, new.recipient_id, 'announcement',
            coalesce(nullif(trim(coalesce(new.message, '')), ''), v_title),
            new.media_url, new.visibility)
    returning id into v_post_id;

    new.post_id := v_post_id;
    new.responded_at := now();

    perform public.lw_notify(new.sender_id, new.recipient_id,
      'announcement_approved', 'Tumhara announcement approve ho gaya 🎉', null, 'dashboard.html');
    perform public.lw_timeline_add(new.sender_id, 'announcement', 'Announcement bheja: ' || v_title,
                                    'announcements', new.id::text, new.visibility = 'public');

  elsif tg_op = 'UPDATE' and new.status = 'rejected' and old.status = 'pending' then
    new.responded_at := now();
    perform public.lw_notify(new.sender_id, new.recipient_id,
      'announcement_rejected', 'Tumhara announcement decline ho gaya', null, 'announcements.html');
  end if;
...
drop trigger if exists lw_notify_announcement_trg on public.announcements;
create trigger lw_notify_announcement_trg before insert or update on public.announcements
  for each row execute function public.lw_notify_announcement();

-- announcements/<user_id>/<file> — public read, owner-only write (posts bucket jaisa hi)
insert into storage.buckets (id, name, public)
  values ('announcements', 'announcements', true)
  on conflict (id) do update set public = true;

drop policy if exists lw_announcements_media_select on storage.objects;
drop policy if exists lw_announcements_media_insert on storage.objects;
drop policy if exists lw_announcements_media_delete on storage.objects;

create policy lw_announcements_media_select on storage.objects
  for select
  using (bucket_id = 'announcements');

create policy lw_announcements_media_insert on storage.objects
  for insert to authenticated
  with check (bucket_id = 'announcements' and (storage.foldername(name))[1] = auth.uid()::text);

create policy lw_announcements_media_delete on storage.objects
  for delete to authenticated
...
-- ------------------------------------------------------------
-- 30. Reports — content moderation. Koi bhi post ya user ko report
--     kar sakta hai; sirf admin padh/action le sakta hai
--     (contact_messages ke insert-anyone/read-admin pattern jaisa hi).
-- ------------------------------------------------------------
create table if not exists public.reports (
  ...
  reason      text not null check (reason in ('spam', 'harassment', 'nudity', 'fake', 'other')),
  details     text,
...
-- koi bhi report bhej sakta hai...
create policy reports_insert_anyone on public.reports
  for insert to authenticated with check (
    reporter_id = auth.uid() and length(coalesce(details, '')) <= 2000
  );

-- ...par padh/action sirf admin le sakta hai
